...
class Report_model extends CI_Model
{
    ...
    public function DuePayment($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $PurchaseFromDate= ($this->input->post('PurchaseFromDate') != '') ? date("Y-m-d", strtotime($this->input->post('PurchaseFromDate'))) : '';
        $PurchaseToDate= ($this->input->post('PurchaseToDate') != '') ? date("Y-m-d", strtotime($this->input->post('PurchaseToDate'))) : '';
    ...
    public function GetPayment($PageSize = 10, $CurrentPage = 1)
    {
        ...
        //$ProjectID = ($this->input->post('ProjectID')!='')?$this->input->post('ProjectID'):-1;
        $ProjectID = ($this->ProjectID != "") ? $this->ProjectID : -1;
        $PaymentFromDate= ($this->input->post('PaymentFromDate') != '') ? date("Y-m-d", strtotime($this->input->post('PaymentFromDate'))) : '';
        $PaymentToDate= ($this->input->post('PaymentToDate') != '') ? date("Y-m-d", strtotime($this->input->post('PaymentToDate'))) : '';
    ...
    function OpportunityListData($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $sql = "call usp_A_GetOpprtunityFromToDate('$PageSize','$CurrentPage','$Name','$MobileNo','$Project','$Source','$FeedbackID','$FromDate','$EndDate')";
        $query = $this->db->query($sql);
        $query->next_result();
        return $query->result();
    }
    ...
    public function FeedbackListData($per_page_record = 10, $page_number = 1)
    {
        $FromDate = getStringClean(($this->input->post('FromDate') != '') ? GetDateInFormat($this->input->post('FromDate'), DATE_FORMAT, DATABASE_DATE_FORMAT) : '');
        $EndDate = getStringClean(($this->input->post('EndDate') != '') ? GetDateInFormat($this->input->post('EndDate'), DATE_FORMAT, DATABASE_DATE_FORMAT) : '');
        $Source = getStringClean(($this->input->post('Source') != '') ? $this->input->post('Source') : 'Phone');
        
        $sql = "call usp_A_GetFeedbackFromToDate( '$per_page_record', '$page_number','$FromDate','$EndDate','$Source')";
        $query = $this->db->query($sql);
        $query->next_result();
        return $query->result();
    }
    ...
    public function VisitDetailsListData($per_page_record = 10, $page_number = 1)
    {
        $FromDate = getStringClean(($this->input->post('FromDate') != '') ? GetDateInFormat($this->input->post('FromDate'), DATE_FORMAT, DATABASE_DATE_FORMAT) : '');
        $EndDate = getStringClean(($this->input->post('EndDate') != '') ? GetDateInFormat($this->input->post('EndDate'), DATE_FORMAT, DATABASE_DATE_FORMAT) : '');
        $ProjectID = $this->ProjectID;
        ...
        $sql = "call usp_A_GetVisitFromToDateDetails( '$per_page_record', '$page_number','$FromDate','$EndDate','$ProjectID','$Source')";
        $query = $this->db->query($sql);
        $query->next_result();
        return $query->result();
    }

    public function ChannelpartnetListData($per_page_record = 10, $page_number = 1)
    {
        $ChannelPartner = getStringClean(($this->input->post('ChannelPartner') != '') ? $this->input->post('ChannelPartner') : '');
        $status_search_value = ($this->input->post('Status') != '') ? $this->input->post('Status') : -1;

        $sql = "call usp_A_GetVisitorByChannelPartner( '$per_page_record', '$page_number','$ChannelPartner','$status_search_value')";
        $query = $this->db->query($sql);
        $query->next_result();
        return $query->result();
    }
