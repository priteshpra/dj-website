...
?>
    <tr id="row_<?php echo $banner->BannerID; ?>">
        <?php if (@$banner->Image != null && (file_exists(str_replace(array('\\', '/system'), array('/', ''), BASEPATH) . BANNER_UPLOAD_PATH . $banner->Image))) {
            $path = $this->config->item('assets') . "uploads/banner/" . $banner->Image;
        } else {
            $path =  $this->config->item('admin_assets') . 'img/noimage.gif';
        }
        ?>
        <td align="center"><img alt="<?php echo $banner->Image; ?>" id="ImagePreivew" src='<?php echo $path; ?>' height='75' width='100'></td>
        <td align="center"><?php echo @$banner->BannerTitle; ?></td>
        <td align="center"><?php echo @$banner->SubTitle1; ?></td>
        <td align="center"><?php echo @$banner->SubTitle2; ?></td>
        <?php
