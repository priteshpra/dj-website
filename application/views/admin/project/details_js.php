<script>
    ...
    $(document).ready(function() {
        ...
        ajax_propertyimagetitle(current_page_size, total_page);
    ...
    function ajax_propertyimagetitle(current_page_size, total_page) {
        DesignationID = '';
        $.ajax({
            type: "post",
            url: base_url + "admin/project/imagetitle/ajax_listing/" + current_page_size + "/" + total_page,
            data: {
                ProjectID: '<?php echo $ProjectID; ?>',
            },
            success: function(data) {
                var obj = JSON.parse(data);
                $('#imagetitle #table_body').html(obj.a);
                $('#imagetitle #table_paging_div').html(obj.b);
            },
            error: function(data) {
                alertify.error('<?php echo label('Something_went_wrong_contact_to_admin'); ?>');
            }
    ...
    $('.tabs .tabclick').on('click', function() {
        ...
        if (myid == "#gallery") {
            ...
            total_page = 1;
            ajax_property(current_page_size, total_page);
        } else if (myid == "#imagetitle") {
            current_page_size = $('#imagetitle #PageSize').val();
            total_page = 1;
            ajax_propertyimagetitle(current_page_size, total_page);
        }
    });
    ...
    $('#imagetitle').on('click', '.pagination_buttons', function() {
        var total_page = $('#imagetitle #PageSize').val();
        var page = $(this).attr('data-page-number');
        ajax_propertyimagetitle(total_page, page);
    });
