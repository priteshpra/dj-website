<script>
    ...
    function ajax_image(current_page_size, total_page) {
        $.ajax({
            type: "post",
            url: base_url + "admin/user/document/ajax_listingimage/" + current_page_size + "/" + total_page,
            data: {
                CustomerPropertyID: '<?php echo $CustomerPropertyID; ?>',
                IsCancelled: '<?php echo $Property->IsCancelled; ?>',
            },
            success: function(data) {
                var obj = JSON.parse(data);
                $('#propertyimage #table_body').html(obj.a);
                $('#propertyimage #table_paging_div').html(obj.b);
            },
            error: function(data) {
                alertify.error('<?php echo label('Something_went_wrong_contact_to_admin'); ?>');
            }
    ...
    function ajax_video(current_page_size, total_page) {
        $.ajax({
            type: "post",
            url: base_url + "admin/user/video/ajax_listing/" + current_page_size + "/" + total_page,
            data: {
                CustomerPropertyID: '<?php echo $CustomerPropertyID; ?>',
                IsCancelled: '<?php echo $Property->IsCancelled; ?>',
            },
            success: function(data) {
                var obj = JSON.parse(data);
                $('#video #table_body').html(obj.a);
                $('#video #table_paging_div').html(obj.b);
            },
            error: function(data) {
                alertify.error('<?php echo label('Something_went_wrong_contact_to_admin'); ?>');
            }
    ...
    $('.tabs .tabclick').on('click', function() {
        ...
        if (myid == "#document") {
            ...
            current_page_size = $("#video #select-dropdown").val();
            total_page = 1;
            ajax_video(current_page_size, total_page);
        } else if (myid == "#propertyimage") {
            current_page_size = $("#propertyimage #select-dropdown").val();
            total_page = 1;
            ajax_image(current_page_size, total_page);
    ...
    $('#video').on('click', '.pagination_buttons', function() {
        var total_page = $('#video #select-dropdown').val();
        var page = $(this).attr('data-page-number');
        ajax_video(total_page, page);
    });
    $('#propertyimage').on('click', '.pagination_buttons', function() {
        var total_page = $('#propertyimage #select-dropdown').val();
        var page = $(this).attr('data-page-number');
        ajax_image(total_page, page);
    });
    ...
    $('#video #select-dropdown').on('change', function() {
        var page = $('#video #select-dropdown').val();
        ajax_video(page, total_page);
    })
    $('#propertyimage #select-dropdown').on('change', function() {
        var page = $('#propertyimage #select-dropdown').val();
        ajax_image(page, total_page);
    })
    ...
    $("#payment #table_body").on('click', '.status_change', function() {
        ...
        $.ajax({
            ...
            data: {
                id: id,
                status: status,
                table_name: "sssm_customerpayment",
                field_name: "CustomerPaymentID"
            },
            success: function(data) {
                var obj = JSON.parse(data);
    ...
    $("#propertyimage #table_body").on('click', '.status_change', function() {
        var tr_id = $(this).attr('data-id');
        var current_status = $(this).attr('data-icon-type');
        $('#propertyimage #row_' + tr_id).find('.' + active_status_icon_class).addClass('hide');
        $('#propertyimage #row_' + tr_id).find('.' + inactive_status_icon_class).addClass('hide');
        $('#propertyimage #row_' + tr_id).find('.' + loading_status_icon_class).removeClass('hide');

        var id = $(this).attr('data-id');
        var status = $(this).attr('data-new-status');
        $.ajax({
            type: "post",
            url: base_url + "admin/user/document/changeStatusImage",
            data: {
                id: id,
                status: status
            },
            success: function(data) {
                var obj = JSON.parse(data);
                if (current_status === 'inactive') {
                    $('#propertyimage #row_' + tr_id).find('.' + active_status_icon_class).removeClass('hide');
                    $('#propertyimage #row_' + tr_id).find('.' + inactive_status_icon_class).addClass('hide');
                    $('#propertyimage #row_' + tr_id).find('.' + loading_status_icon_class).addClass('hide');
                } else {
                    $('#propertyimage #row_' + tr_id).find('.' + active_status_icon_class).addClass('hide');
                    $('#propertyimage #row_' + tr_id).find('.' + inactive_status_icon_class).removeClass('hide');
                    $('#propertyimage #row_' + tr_id).find('.' + loading_status_icon_class).addClass('hide');
                }
                alertify.success(obj.message);
            }
    ...
    $("#propertyimage #table_body").on('click', '.delete', function() {
        $("#model_title").html("Customer Property Image");
        var id = $(this).attr('data-id');
        $.ajax({
            type: "post",
            url: base_url + "admin/user/document/deleteImage",
            data: {
                id: id
            },
            success: function(data) {
