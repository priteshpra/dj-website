...
if(!empty($data_array)) { 
	...
	<tr id="row_<?php echo $data->CustomerPropertyID; ?>">
		...
		<?php 
		if(@$this->payment_tmp->is_view == 1){
		?>
			<a href="<?php echo site_url('admin/user/property/details/'.$data->CustomerPropertyID . "#payment");?>">
		<?php } ?>
		...
		<td class="action center">
			...
			<a class="btn-floating waves-effect waves-light blue m-r-5 <?php echo ($data->IsCancelled==0 && $data->is_edit==1)?'':' disabled-edit ';?>" href="<?php echo ($data->IsCancelled==0 && $data->is_edit==1)?site_url('admin/user/property/edit/'.$CustomerID."/".$data->CustomerPropertyID):"javascript:void(0)"; ?>" ><i title="Edit" class="<?php echo EDIT_ICON_CLASS; ?>"></i>
			</a>
			...
			<a class="btn-floating waves-effect waves-light red m-r-5 <?php echo ($data->IsCancelled==0 && $data->is_cancel == 1)?' CancelProperty ':' disabled-edit ';?>" href="javascript:void(0);" data-id="<?php echo $data->CustomerPropertyID; ?>"><i title="Cancel" class="mdi-content-clear"></i>
			</a>
