...
if (!function_exists('getPropertyCombobox')) {
    function getPropertyCombobox($Selected = 0, $ProjectID = null, $Select2 = 0, $Type = "")
    {
        $CI = &get_instance();
        $data = array();
        if ($Type == "All") {
...
if (!function_exists('getFeedbackComboBoxValueID')) {
    function getFeedbackComboBoxValueID($Selected = 0)
    {
        $CI = &get_instance();
        $data = array();
        $Selected = (int)$Selected;
        ...
        $data['all_data'] = $CI->feedback_model->ListData(-1, 1);
        //$data['Selected'] = $Selected;
        return $CI->load->view('common_view_files/feedback_combo_box_value_id', $data, TRUE);
    }
}
