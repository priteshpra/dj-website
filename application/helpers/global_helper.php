...
if (!function_exists('sendSMS')) {
    ...
    function sendSMS($MobileNo, $MsgText)
    {
        ...
        /* $api_key = '460420119787B6';
        // $contacts = '8866739817';
        $from = 'AMALGA';
        $MsgText = urlencode($MsgText);
        //Submit to server
        //echo "http://sms.letsenkindle.com/app/smsapi/index.php?key=".$api_key."&campaign=11028&routeid=7&type=text&contacts=".$MobileNo."&senderid=".$from."&msg=".$MsgText;
        $ch = curl_init();
        curl_setopt($ch,CURLOPT_URL, "http://sms.letsenkindle.com/app/smsapi/index.php");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "key=".$api_key."&campaign=11028&routeid=7&type=text&contacts=".$MobileNo."&senderid=".$from."&msg=".$MsgText);
        ...
        // $contacts = '8866739817';
        $from = 'Amalga';
        $template_id = '1507162211365048090'; // crm
        $MsgText = urlencode($MsgText);
        //$sms_text = urlencode('Dear Snehal Thank you for visit Shreeya Amalga. We are happy to see you again at Amalga. Pleaste fill free to connect with us. 9913641111');

        //echo "http://sms.letsenkindle.com/app/smsapi/index.php?"."key=".$api_key."&campaign=11028&routeid=7&type=text&contacts=".$MobileNo."&senderid=".$from."&msg=".$MsgText."&template_id=".$template_id;
        ...
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "key=".$api_key."&campaign=11028&routeid=7&type=text&contacts=".$MobileNo."&senderid=".$from."&msg=".$MsgText."&template_id=".$template_id);
        $response = curl_exec($ch);
        curl_close($ch);
        echo $response;
 */
        ...
        $MsgText = urlencode($MsgText);
        //$sms_text = urlencode('Dear snehal, Thank you for visit Shreeya Amalga. We are happy to see you again at our site. Please feel free to connect with us. 9913641111');
...
if (!function_exists('sendSMSOLD')) {
    //Define sendPushNotification function
    function sendSMSOLD($MobileNo, $MsgText)
    {
        ...
        if (@$config[0]->SMSUserName != '' && @$config[0]->SMSPassword != ''  && @$config[0]->SMSSenderID != '' && @$config[0]->SMSType != '' && @$MobileNo != '' && @$MsgText != '') {
            ...
            $url = "http://203.129.203.243/blank/sms/user/urlsms.php?username=shreeyaama&pass=Wh0-uV9$&senderid=AMALGA&dest_mobileno=" . $MobileNo . "&msgtype=UNI&message=" . urlencode($MsgText) . "&response=Y";
...
if (!function_exists('GetDateTimeInFormat')) {
    function GetDateTimeInFormat($Date, $CurDateFormay = DATABASE_DATE_TIME_FORMAT, $Convert_Format = DATE_TIME_FORMAT)
    {
        ...
        if ($r_date) {
            ...
            $n_date = date($Convert_Format, strtotime($Date));
        }
        return $n_date;
    }
}
