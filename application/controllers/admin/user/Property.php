  ...
  public function __construct() 
    {
        ...
        $tmp =  $this->db->query("CALL usp_A_GetRoleModuleByID(" . $this->UserRoleID . ",18," . $this->ProjectID .")");
        $tmp->next_result();
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if($status_submit==1){
                        ...
                        if (@$res->ID) {
                                ...
                                $_DeviceResult = $this->common_model->getDeviceByEmployee($user_ID,'CustomerProperty',@$res->ProjectID);
                                if(!empty($_DeviceResult) && !isset($_DeviceResult[0]->Message)){  
                                    $Message = $res->EmployeeFirstName.' '.$res->EmployeeLastName.' has added property of '.$res->FirstName.' '.$res->LastName.'(customer).';
                                        $NoOfUser = array();
                                        ...
                                        foreach ($NoOfUser as $val) {
                                            $addnotification = $this->common_model->getInlineQuery("SELECT Fn_A_AddNotification('".$val->UserID."', '".$Message."' , '".$user_ID."', '".$res->ID."','AddCustomerProperty') as IsNotificationAdded, 'Success' as Status");
                                        }
                                    foreach ($_DeviceResult as $d_val) { 
                                        //pr($d_val->DeviceTokenID);pr($rc_val->Message);
                                        if(!empty($d_val->DeviceTokenID) && !empty($Message)){  

                                            // $addnotification = $this->common_model->getInlineQuery("SELECT Fn_A_AddNotification('".$d_val->UserID."', '".$Message."' , '".$user_ID."', '".$res->ID."','AddCustomerProperty') as IsNotificationAdded, 'Success' as Status");
                            ...
                            if($data['paymentflag'] == 1 && $tmp_mod->is_insert == 1){
                                ...
                                redirect($this->config->item('base_url') . 'admin/user/payment/add/'.$res->ID);
                            }
