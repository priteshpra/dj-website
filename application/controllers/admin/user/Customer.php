        ...
        if ($this->input->post()) {
                ...
                if($this->form_validation->run() == TRUE){
                    ...
                    if(@$res->ID){
                        ...
                        $_DeviceResult = $this->common_model->getDeviceByEmployee($user_ID,'Customer','-1');
                        if(!empty($_DeviceResult) && !isset($_DeviceResult[0]->Message)){  
                            $Message = $res->EmployeeFirstName.' '.$res->EmployeeLastName.' has added '.$this->input->post('FirstName').' '.$this->input->post('LastName').'(customer).';
                                    ...
                                    foreach ($NoOfUser as $val) {
                                        $addnotification = $this->common_model->getInlineQuery("SELECT Fn_A_AddNotification('".$val->UserID."', '".$Message."' , '".$user_ID."', '".$res->ID."','AddCustomer') as IsNotificationAdded, 'Success' as Status");
                                    }
                        ...
                        redirect($this->config->item('base_url') . 'admin/user/property/add/'.$CustomerID);
                    }else{
