...
class Blog extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/blog/list', $array);
        $data['page_level_js'] = $this->load->view('admin/blog/list_js', NULL, TRUE);
    ...
    public function ajax_listing($per_page_record = 10, $page_number = 1)
    {
        ...
        $res['cms'] = $this->blog_model->listData($per_page_record, $page_number);
        if (empty($res['cms']))
            $res['total_records'] = 0;
        else
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/blog/ajax_listing', $res, TRUE);
    ...
    public function add()
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        redirect($this->config->item('base_url') . 'admin/blog/add');
                    }
                } else {
            ...
            $res['page_level_js'] = $this->load->view('admin/blog/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL)
    {
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/blog/edit/' . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/blog/edit/' . $ID);
            ...
            $data['cms'] = $this->blog_model->getCmsByID($ID);
            $data['loading_button'] = getLoadingButton();
    ...
    function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->blog_model->changeStatus($this->input->post());
                if ($res) {
