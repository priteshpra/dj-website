...
class Appointment extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/appointment/list', $array);
        $data['page_level_js'] = $this->load->view('admin/appointment/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/appointment/add';
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $res['data_array'] = $this->appointment_model->ListData($PageSize, $CurrentPage);
    ...
    public function add()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $url = site_url("admin/artist/add");
                $config = array(
                    "max_width" => PROJECT_VIDEO_MAX_WIDTH,
                    "max_height" => PROJECT_VIDEO_MAX_HEIGHT,
                    'max_size' => PROJECT_VIDEO_MAX_SIZE,
                    'path' => PROJECT_VIDEO_UPLOAD_PATH,
                ...
                if (@$res->ID) {
                    ...
                    redirect(site_url('admin/artist/add'));
                }
            } else {
        ...
        $data['country'] = getCountryCombobox(0);
        $data['state'] = getStateCombobox(0);
        $data['city'] = getCityCombobox(0);
        $data['artistcategory'] = getArtistCatCombobox(0);
        $this->load->view('admin/artist/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/artist/add_edit_js', NULL, TRUE);
    ...
    {
        $data = $res = array();
        $data['data'] = $this->appointment_model->GetByID($ID);
        // echo "<pre>";
        // print_r($data['data']);
        // die;
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $url = site_url("admin/artist/edit" . $ID);
                $config = array(
                    "max_width" => PROJECT_VIDEO_MAX_WIDTH,
                    "max_height" => PROJECT_VIDEO_MAX_HEIGHT,
                    'max_size' => PROJECT_VIDEO_MAX_SIZE,
                ...
                // $data['FilePath'] = FileUploadURL("FilePath", "editImageURL", $config, '', $url);
                $data['ID'] = $ID;
                $res = $this->appointment_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url('admin/artist'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/artist/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/artist/edit/' . $ID));
    ...
    public function export_to_excel()
    {
        ...
        $array['data'] = $this->appointment_model->ListData(-1, 1);
        //activate worksheet number 1
        $this->excel->setActiveSheetIndex(0);
        //name the worksheet
        $this->excel->getActiveSheet()->setFirstName('Export Data');
        $this->excel->setActiveSheetIndex(0);
        //Set Header Style
        $col = 0;
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->appointment_model->changeStatus($this->input->post());
                if ($res) {
    ...
    public function combobox()
    {
        if ($this->input->get()) {
            $json = array();
            $_POST['FirstName'] = ($this->input->get('q') == "") ? '' : $this->input->get('q');
            $_POST['Status'] = 1;
            $data = $this->appointment_model->ListData(10, 1);
            foreach ($data as $key => $value) {
                if (@$value->Message)
                    break;
                $json[] = ['id' => $value->CategoryID, 'text' => $value->FirstName];
            }
