...
class Artist extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/artist/list', $array);
        $data['page_level_js'] = $this->load->view('admin/artist/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/artist/add';
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $res['data_array'] = $this->artist_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/artist/ajax_listing', $res, TRUE);
    ...
    {
        $data = $res = array();
        $data['data'] = $this->artist_model->GetByID($ID);
        // echo "<pre>";
        // print_r($data['data']);
        // die;
    ...
    public function export_to_excel()
    {
        ...
        $array['data'] = $this->artist_model->ListData(-1, 1);
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->artist_model->changeStatus($this->input->post());
                if ($res) {
    ...
    public function combobox()
    {
        if ($this->input->get()) {
            ...
            $_POST['Status'] = 1;
            $data = $this->artist_model->ListData(10, 1);
