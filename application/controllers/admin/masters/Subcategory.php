...
class Subcategory extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/subcategory/add';
    ...
    public function add()
    {
        ...
        $data['catData'] = $this->category_model->ListData(100, 1);
        $this->load->view('admin/includes/header');
        $data['page_name'] = 'add';
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->subcategory_model->changeStatus($this->input->post());
                if ($res) {
    ...
    public function combobox()
    {
        if ($this->input->get()) {
            ...
            $_POST['Status'] = 1;
            $data = $this->subcategory_model->ListData(10, 1);
            foreach ($data as $key => $value) {
                if (@$value->Message)
                    break;
                $json[] = ['id' => $value->CategoryID, 'text' => $value->SubCategoryName];
            }
