...
class Section extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/masters/section/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/section/list_js', NULL, TRUE);
    ...
    public function ajax_listing($per_page_record = 10, $page_number = 1)
    {
        ...
        $res['cms'] = $this->section_model->listData($per_page_record, $page_number);
        if (empty($res['cms']))
            $res['total_records'] = 0;
        else
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/section/ajax_listing', $res, TRUE);
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $res = $this->section_model->insert($this->input->post());

                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/masters/section');
                    } else {
                        $error_array = array(
                        ...
                        redirect($this->config->item('base_url') . 'admin/masters/section/add');
                    }
                } else {
            ...
            $this->load->view('admin/masters/section/add_edit', $data);
            $res['page_level_js'] = $this->load->view('admin/masters/section/add_edit_js', NULL, TRUE);
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $data['ID'] = $ID;
                    $res = $this->section_model->update($data);
                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/masters/section');
                    } else {
                        $error_array = array(
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/section/edit/' . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/section/edit/' . $ID);
            ...
            $data['cms'] = $this->section_model->getCmsByID($ID);
            $data['loading_button'] = getLoadingButton();
    ...
    function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->section_model->changeStatus($this->input->post());
                if ($res) {
