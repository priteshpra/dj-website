...
class Brand extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/masters/brand/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/brand/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/brand/add';
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $res['data_array'] = $this->brand_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/brand/ajax_listing', $res, TRUE);
    ...
    public function add()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $url = site_url("admin/masters/brand/add");
                $config = array(
                    "max_width" => BRAND_MAX_WIDTH,
                    "max_height" => BRAND_MAX_HEIGHT,
                    'max_size' => BRAND_MAX_SIZE,
                    'path' => BRAND_UPLOAD_PATH,
                    'allowed_types' => BRAND_ALLOWED_TYPES,
                    'tpath' => BRAND_THUMB_UPLOAD_PATH,
                    'twidth' => BRAND_THUMB_MAX_WIDTH,
                    'theight' => BRAND_THUMB_MAX_HEIGHT
                );

                $data = $this->input->post();
                $data['LogoFilePath'] = FileUploadURL("LogoFilePath", "editImageURL", $config, '', $url);
                $res = $this->brand_model->Insert($data);
                if (@$res->ID) {
                    redirect(site_url('admin/masters/brand'));
                } else {
                    ...
                    redirect(site_url('admin/masters/brand/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/brand/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/brand/add_edit_js', NULL, TRUE);
    ...
    {
        $data = $res = array();
        $data['data'] = $this->brand_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $url = site_url("admin/masters/brand/edit" . $ID);
                $config = array(
                    "max_width" => BRAND_MAX_WIDTH,
                    "max_height" => BRAND_MAX_HEIGHT,
                    'max_size' => BRAND_MAX_SIZE,
                ...
                $data['LogoFilePath'] = FileUploadURL("LogoFilePath", "editImageURL", $config, '', $url);
                $data['ID'] = $ID;
                $res = $this->brand_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url('admin/masters/brand'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/brand/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/brand/edit/' . $ID));
    ...
    public function export_to_excel()
    {
        ...
        $array['data'] = $this->brand_model->ListData(-1, 1);
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->brand_model->changeStatus($this->input->post());
                if ($res) {
    ...
    public function combobox()
    {
        if ($this->input->get()) {
            $json = array();
            $_POST['BrandName'] = ($this->input->get('q') == "") ? '' : $this->input->get('q');
            $_POST['Status'] = 1;
            $data = $this->brand_model->ListData(10, 1);
            foreach ($data as $key => $value) {
                if (@$value->Message)
                    break;
                $json[] = ['id' => $value->CategoryID, 'text' => $value->BrandName];
            }
